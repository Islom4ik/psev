...
bot.command('rules', async ctx => {
    try {
        await ctx.replyWithPhoto({source: './Previewrules.jpg'}, {...Markup.inlineKeyboard([[Markup.button.callback('🖲 Запустить игру', 'stgame')]]), caption: '👁‍🗨 Два игрока случайным образом получают карты. Лишь один из них будет знать, что ему попалось — деньги или пустышка. Задача этого игрока — убедить соперника обменяться, либо оставить всё как есть. В раунде побеждает тот, у кого на руках будет карта с деньгами. Игра продлится, пока на балансе одного из игроков не окажется пусто.'})
    }catch(e) {
        console.error(e);
    }
})
